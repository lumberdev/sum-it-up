...
export default async function handler(
  ...
  try {
    ...
    if (type === "song" || type === "article") openAiResponse = await callWithUrl(url, wordLimit, type, HOST_URL);
...
async function callWithUrl(url: string, wordLimit: number, type: ContentType, host?: string) {
  try {
    const CHUNK_LENGTH = 500;
    // await fetchRetry(`/readability?url_resource=${url}`, 100, 3)
    const innerResponse = await fetch(`${host}/api/readability?url_resource="${url}"&chunk_length=${CHUNK_LENGTH}`);
