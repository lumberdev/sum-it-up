...
export default async function handler(
  req: NextApiRequest,
  res: NextApiResponse<DocumentResponseData | ErrorMessage>
) {
  ...
  const chunkLength = +getStringOrFirst(query.chunk_length).replace(
    /['"]+/g,
    ""
  );
  ...
  if (method === "GET") {
    if (!urlResource)
      res
        .status(400)
        .json({ message: "Please provide a valid URL", code: 400 });
    ...
    try {
      ...
      if (result)
        return res.status(200).json({
          url: urlResource,
          chunkedTextContent: textToChunks(
            result.textContent,
            !isNaN(chunkLength) ? chunkLength : 2000
          ),
          ...result,
        });
      ...
      return res
        .status(400)
        .json({ message: "Could not retrieve parsed data for URL", code: 400 });
    } catch (error) {
